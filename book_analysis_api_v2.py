...
"""
书籍销售分析API - 数据库版本
FastAPI接口，提供基于ISBN的书籍销售数据分析，使用SQLite数据库
"""
import asyncio
import os
...
from database import DatabaseManager, SalesRepository
...
# 初始化FastAPI应用
app = FastAPI(
    title="书籍销售分析API (数据库版)",
    description="基于孔夫子旧书网的书籍销售数据分析接口，使用SQLite数据库",
    version="2.0.0"
)
...
async def save_to_database(sales_data: List[Dict], isbn: str):
    """将销售数据保存到数据库"""
    if not sales_data:
        return
    ...
    for sale in sales_data:
        db_data.append({
            ...
            'price': float(sale.get('price', 0)) if sale.get('price') else 0,
            'quality': sale.get('quality', ''),
            'analyzed_at': current_time
        })
    
    try:
        await sales_repo.save_sales_data(db_data)
        print(f"已保存 {len(db_data)} 条销售记录到数据库")
    except Exception as e:
        print(f"保存到数据库失败: {e}")
...
async def analyze_book(request: BookAnalysisRequest):
    ...
    try:
        ...
        # 保存到数据库
        if sales_data:
            await save_to_database(sales_data, isbn)
...
async def migrate_csv_data():
    ...
    try:
        ...
        return {"message": "CSV数据迁移完成", "success": True}
    except Exception as e:
