...
### 智能窗口池策略
系统采用**首选窗口机制**，优化了爬虫的窗口使用策略：

#### 核心特性
- **固定首选窗口**：业务优先使用同一个窗口，减少不必要切换
- **智能限流检测**：基于aiolimiter精确控制访问频率
- **自动故障切换**：首选窗口被限流时自动切换到其他窗口
- **窗口状态监控**：实时追踪窗口可用性和限制状态

#### 工作流程
1. **优先使用首选窗口** - 业务请求默认使用固定窗口
2. **限流触发切换** - 检测到频率限制时自动选择其他窗口
3. **设置新首选窗口** - 将可用窗口设为新的首选窗口
4. **全部受限等待** - 所有窗口都被限制时保持原有等待机制
...
```bash
# 查看窗口池状态
curl "http://localhost:8282/window-pool/status"

# 日志关键词
grep "窗口切换" logs/app.log
grep "首选窗口" logs/app.log
```

## 更新日志

### v2.2 (2025-08) 🆕
- 🔧 **窗口池优化**：实现首选窗口机制，提升爬虫效率
  - 业务优先使用固定窗口，减少不必要的窗口切换
  - 智能限流检测，触发限制时自动切换窗口
  - 增强窗口状态监控和日志记录
  - 优化窗口池管理，提高系统稳定性
